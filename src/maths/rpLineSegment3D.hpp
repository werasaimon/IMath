#ifndef RPLINESEGMENT3D_H
#define RPLINESEGMENT3D_H
...
template<class T> class rpLineSegment3D;

template<class T>
T DistanceSquared( const rpLineSegment3D<T>& seg0, const rpLineSegment3D<T>& seg1, T& s_c, T& t_c );

template<class T>
T DistanceSquared( const rpLineSegment3D<T>& segment, const rpLine3D<T>& line,  T& s_c, T& t_c );

//-------------------------------------------------------------------------------
//-- Classes --------------------------------------------------------------------
//-------------------------------------------------------------------------------
template<class T>
class rpLineSegment3D
{
public:
    ...
    rpLineSegment3D( const rpVector3D<T>& endpoint0, const rpVector3D<T>& endpoint1 )
     : mOrigin( endpoint0 ),
    ...
    SIMD_INLINE ~rpLineSegment3D() {}

    // copy operations
    rpLineSegment3D(const rpLineSegment3D<T>& other)
    ...
    // Assigment operator
    //-----------------------------------------------------------------------------
    rpLineSegment3D& operator=(const rpLineSegment3D<T>& other)
    {
        // if same object
         if ( this == &other )
             return *this;
    ...
    SIMD_INLINE rpVector3D<T> GetEndpoint0() const { return mOrigin; }
    SIMD_INLINE rpVector3D<T> GetEndpoint1() const { return mOrigin + mDirection; }
    SIMD_INLINE rpVector3D<T> GetCenter() const { return mOrigin + T(0.5f)*mDirection; }

    // ---------------------------------------------------------------------------
    // Returns the two endpoints
    //-----------------------------------------------------------------------------
    void Get( rpVector3D<T>& end0, rpVector3D<T>& end1 ) const
    {
        end0 = mOrigin;
    ...
    // manipulators
    void Set( const rpVector3D<T>& end0, const rpVector3D<T>& end1 )
    {
        mOrigin = end0;
    ...
    //----------------------------------------------------------------------------
    bool operator==( const rpLineSegment3D<T>& segment ) const
    {
    ...
    //-----------------------------------------------------------------------------
    rpLineSegment3D Transform( T scale, const rpQuaternion<T>& quat, const rpVector3D<T>& translate ) const
    {
        rpLineSegment3D<T> segment;
        rpMatrix3x3<T>  rotate = quat.getMatrix();
    ...
    // Transforms segment into new space
    //-----------------------------------------------------------------------------
    rpLineSegment3D Transform( T scale, const  rpMatrix3x3<T>& rotate, const rpVector3D<T>& translate ) const
    {
        rpLineSegment3D<T> segment;
    ...
    //-----------------------------------------------------------------------------
    static T DistanceSquared( const rpLineSegment3D<T>& segment0, const rpLineSegment3D<T>& segment1,  T& s_c, T& t_c )
    {
            // compute intermediate parameters
            rpVector3D<T> w0 = segment0.mOrigin - segment1.mOrigin;
            ...
            rpVector3D<T> wc = w0 + s_c*segment0.mDirection - t_c*segment1.mDirection;
            return wc.dot(wc);
    }
    ...
    // Based on article and code by Dan Sunday at www.geometryalgorithms.com
    //-----------------------------------------------------------------------------
    static T DistanceSquared( const rpLineSegment3D<T>& segment,  const rpLine3D<T>& line,  T& s_c, T& t_c )
    {
        // compute intermediate parameters
        rpVector3D<T> w0 = segment.mOrigin - line.GetOrigin();
        ...
        if ( IsZero(denom) )
        {
            ...
            t_c = e/c;
            // compute difference vector and distance squared
            rpVector3D<T> wc = w0 - t_c*line.GetDirection();
            return wc.dot(wc);
        }
        else
        {
            ...
            // compute difference vector and distance squared
            rpVector3D<T> wc = w0 + s_c*segment.mDirection - t_c*line.GetDirection();
            return wc.dot(wc);
        }
    }

    static T DistanceSquared( const rpLine3D<T>& line,  const rpLineSegment3D<T>& segment, T& s_c, T& t_c )
    {
    ...
    //-----------------------------------------------------------------------------
    static T DistanceSquared( const rpLineSegment3D<T>& segment,  const rpVector3D<T>& point,  T& t_c )
    {
        rpVector3D<T> w = point - segment.mOrigin;
    ...
    //-----------------------------------------------------------------------------
    static void ClosestPoints( const rpLineSegment3D<T>& segment0,  const rpLineSegment3D<T>& segment1 , rpVector3D<T>& point0, rpVector3D<T>& point1 )
    {
            // compute intermediate parameters
            rpVector3D<T> w0 = segment0.mOrigin - segment1.mOrigin;
            T a = segment0.mDirection.dot( segment0.mDirection );
    ...
    //-----------------------------------------------------------------------------
    static void ClosestPoints( const rpLineSegment3D& segment,  const rpLine3D<T>& line , rpVector3D<T>& point0, rpVector3D<T>& point1 )
    {
     ...
     */
     friend std::ostream& operator <<(std::ostream& oss, const rpLineSegment3D<T>& rhs)
     {
